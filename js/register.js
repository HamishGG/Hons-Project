...
function checkUser(username,email,callback) {
	...
	var XHR=createXHR();
	XHR.open("POST","php/checkUser.php",true);
...
function checkForm(e) {
	...
	with(theform) {
		...
		if(checkEmpty(username.value)) 
			{ usernameFb.innerHTML="Please add your desired username"; iscomplete=false; }
		else
		...
		if(checkEmail(emailadd.value))
			{ emailFb.innerHTML="Please add a valid email"; iscomplete=false; }
		else
			{ emailFb.innerHTML=""; }
	} 
